...
const ButtonToTemplateStateToggle = ({
  ToggleState,
  state,
  title,
}: ButtonToTemplateStateToggleProps) => {
  return (
    <div
      className={styled['button-to-template-state-toggle']}
      aria-labelledby="botão para abrir menu de alteração de informações do usuário"
      onClick={() => ToggleState()}
      data-isOpen={state}
    >
      <p className={styled['title']}>{state ? 'Fechar' : title}</p>
      <motion.div layout className={styled['small-button']}>
