...
export const ModalSelectCategoriesInserted = () => {
  const { sportAndCourseSelected, genderAndSportCategorySelected } =
    PageDisputaRegistrarConnection();
  const { sportCategorySelected, genderCategorySelected } =
    genderAndSportCategorySelected;
  const { sportSelected } = sportAndCourseSelected;

  const { modalSelectCategories, modalToConfigGame } =
    usePageRegistrarDisputa();
